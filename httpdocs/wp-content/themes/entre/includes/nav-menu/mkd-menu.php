...
add_filter( 'wp_setup_nav_menu_item', 'entre_mikado_add_custom_nav_fields' );

// save menu custom fields
add_action( 'wp_update_nav_menu_item', 'entre_mikado_update_custom_nav_fields', 10, 3 );

// edit menu walker
add_filter( 'wp_edit_nav_menu_walker', 'entre_mikado_edit_walker', 10, 2 );

if ( ! function_exists( 'entre_mikado_add_custom_nav_fields' ) ) {
	function entre_mikado_add_custom_nav_fields( $menu_item ) {
 ...
 * @return      void
 */
if ( ! function_exists( 'entre_mikado_update_custom_nav_fields' ) ) {
	function entre_mikado_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
		$check = array(
			'anchor',
			'nolink',
			'hide',
 ...
 * @return      void
 */
if ( ! function_exists( 'entre_mikado_edit_walker' ) ) {
	function entre_mikado_edit_walker( $walker, $menu_id ) {
		return 'EntreMikado_Walker_Nav_Menu_Edit_Custom';
	}
}

if ( ! function_exists( 'entre_mikado_include_custom_walkers' ) ) {
	function entre_mikado_include_custom_walkers() {
		...
		do_action( 'entre_mikado_include_custom_walkers_nav' );
	}
	
	add_action('after_setup_theme', 'entre_mikado_include_custom_walkers');
}