(function($){
    $(document).ready(function() {
        ...
        mkdInitSelectChange();
        mkdInitSwitch();
        mkdInitSaveCheckBoxesValue();
        mkdCheckBoxMultiSelectInitState();
        mkdInitCheckBoxMultiSelectChange();
        mkdInitTooltips();
        mkdInitColorpicker();
        mkdInitRangeSlider();
    ...
    function mkdChangedInput () {
        ...
        $('.mkd-tabs-content form.mkd_ajax_form:not(.mkd-import-page-holder):not(.mkd-backup-options-page-holder) .field.switch label:not(.selected)').on('click',  function() {
            $('.mkd-input-change').addClass('yes');
        });
        ...
        $('.mkd-tabs-content form.mkd_ajax_form:not(.mkd-import-page-holder):not(.mkd-backup-options-page-holder) #anchornav input').on('click', function() {
            if ($('.mkd-input-change.yes').length) {
    ...
    function mkdInitSelectChange() {
        ...
        var switchers = $('select.mkd-switcher');
        ...
        function changeActions(selectField, valueSelected, initialCall) {
            ...
            if (switchType === 'single_yesno') {
                ...
                if (switchEnabled === valueSelected) {
                    switchers.addClass('mkd-switch-single-mode');
                    switchers.attr('data-switch-selector', switchProperty);
                } else {
                    switchers.removeClass('mkd-switch-single-mode');
    ...
    function mkdInitSwitch() {
        ...
        yesNoElements.each(function () {
            var element = $(this);
            if (element.parents('.mkd-repeater-field') && !element.find('input[type="hidden"]').val()) {
        ...
        $(".cb-enable").on('click', function(){
            ...
            if(parent.hasClass('mkd-switch-single-mode')) {
    ...
    function mkdInitSaveCheckBoxesValue(){
        var checkboxes = $('.mkd-single-checkbox-field');
        checkboxes.change(function(){
            mkdDisableHidden($(this));
        });
        checkboxes.each(function(){
            mkdDisableHidden($(this));
        });
        function mkdDisableHidden(thisBox){
            if(thisBox.is(':checked')){
                thisBox.siblings('.mkd-checkbox-single-hidden').prop('disabled', true);
            }else{
                thisBox.siblings('.mkd-checkbox-single-hidden').prop('disabled', false);
            }
        }
    }

    function mkdCheckBoxMultiSelectInitState(){
        var element = $('input[type="checkbox"].dependence.multiselect');
        if(element.length){
            element.each(function() {
                var thisItem = $(this);
                mkdInitCheckBox(thisItem);
            });
        }
    }

    function mkdInitCheckBoxMultiSelectChange() {
        var element = $('input[type="checkbox"].dependence.multiselect');
    ...
    function mkdInitCheckBox(checkBox){
    ...
    function mkdInitGalleryUploader() {
        ...
        wp.media.customlibEditGallery1 = {
            frame: function() {
                ...
                this._frame.on('update', function() {
                    ...
                    jQuery.ajax({
                        type: "post",
                        url: ajaxurl,
                        data: "action=entre_mikado_gallery_upload_get_images&ids=" + ids,
            ...
            init: function() {
                $mkd_upload_button.on('click', function(event) {
                ...
                $mkd_clear_button.on('click', function(event) {
    ...
    function mkdInitPortfolioItemsBox() {
        ...
        $('.mkd-portfolio-add a.mkd-add-item').on('click', function (event) {
            event.preventDefault();
    ...
    function mkdInitPortfolioImagesVideosBox() {
        ...
        $('a.mkd-add-image').on('click', function (e) {
            e.preventDefault();
        ...
        $('a.mkd-add-video').on('click', function (e) {
            e.preventDefault();
    ...
    */
    ...
    function mkdInitSlideElementItemsBox() {
        $('.mkd-slide-element-add a.mkd-add-item').on('click', function (event) {
    ...
    */
    function mkdInitAjaxForm() {
        $('#mkd_top_save_button').on('click',  function() {
        ...
        $(document).delegate(".mkd_ajax_form", "submit", function (a) {
            var b = $(this);
            var c = {
                action: "entre_mikado_save_options"
            };
            jQuery.ajax({
                url: ajaxurl,
                cache: !1,
    ...
    function mkdInitDatePicker() {
		var holder = $('.mkd-input.datepicker');
    ...
    function mkdRepeater(){
        var wrapper = $('.mkd-repeater-wrapper');
        ...
        function initCoreRepeater(wrapper) {
            ...
            //Init add new button
            var addNew = wrapper.find('> .mkd-repeater-add .mkd-clone'); // add new button
        ...
        function initRemoveRow(wrapper){
            var removeBtn = wrapper.find('.mkd-clone-remove');
            removeBtn.on('click', function (e) {
                e.preventDefault();
                var thisRemoveBtn = $(this);
                var parentRow = thisRemoveBtn.closest('.mkd-repeater-fields-row');
                var fieldsHolder = thisRemoveBtn.closest('.mkd-repeater-fields-holder');
                var parentChildRepeater = !!fieldsHolder.hasClass('mkd-enable-pc');
                var thisHolderRows;

                if(fieldsHolder.hasClass('mkd-table-layout')) {
                    thisHolderRows = fieldsHolder.find('tbody tr.mkd-repeater-fields-row');
                } else {
                    if(parentChildRepeater) {
                        var name = thisRemoveBtn.data("name");
                        thisHolderRows = fieldsHolder.find('> .mkd-repeater-fields-row[data-name=' + name + ']');
                    } else {
                        thisHolderRows = fieldsHolder.find('> .mkd-repeater-fields-row');
                    }
                }
        ...
        function initEmptyRow(wrapper) {
            var fieldsHolder = wrapper.find('> .mkd-repeater-fields-holder');
            var thisHolderRows;
            if(fieldsHolder.hasClass('mkd-table-layout')) {
                thisHolderRows = fieldsHolder.find('tbody tr.mkd-repeater-fields-row');
            } else {
            ...
            thisHolderRows.each(function() {
                var row = $(this);
                if (row.hasClass('mkd-initially-hidden')) {
                    row.hide();
                }
            });
        }
        ...
        function initCloneRow(wrapper, button) {
            ...
            var rows;
            if(fieldsHolder.hasClass('mkd-table-layout')) {
                rows = fieldsHolder.find('tbody tr.mkd-repeater-fields-row');
            } else {
                if(parentChildRepeater) {
                    var name = button.data("name");
                    rows = fieldsHolder.find('> .mkd-repeater-fields-row[data-name=' + name + ']');
                } else {
                    rows = fieldsHolder.find('> .mkd-repeater-fields-row');
                }
            }
            ...
            if (append) {
                ...
                child.find('.mkd-repeater-field').each(function () {
                    var thisField = $(this);
                    thisField.find('select').each(function () {
                        var thisInput = $(this);
                        if(thisInput.hasClass('mkd-select2')) {
                            ...
                            if (thisInput.hasClass('my-color-field')) { // if input type is color picker
                                ...
                                if (typeof id !== 'undefined') {
                                    ...
                                    var textareaID = thisInput.attr('id'),
                                        ...
                                        textareaHolder = thisInput.parents('.mkd-repeater-field').first(),
                                        idForTinyMC = id.substring('mkd_'.length,id.length+1),
                ...
                if (yesNoSwitcher) {
                    mkdInitSwitch(); //init yes no switchers
                }
                if (select2) {
                    mkdSelect2(); //init select2 script
                }
                if (datepicker) {
                    mkdInitDatePicker(); //init select2 script
                }
            }
        }
    ...
    function mkdInitSortable() {
        var sortingHolder = $('.mkd-sortable-holder');
        var enableParentChild = sortingHolder.hasClass('mkd-enable-pc');
        sortingHolder.sortable({
            handle: '.mkd-repeater-sort',
            cursor: 'move',
            ...
            sort: function(event, ui) {
                if(enableParentChild) {
                    ...
                    if (pos >= 75 && !ui.helper.hasClass('second-level') && !ui.helper.hasClass('mkd-sort-parent')) {
                        ui.placeholder.removeClass('placeholder');
                        ui.placeholder.addClass('placeholder-sub');
                        ui.helper.addClass('second-level');
                    }
                    else if (pos < 30 && ui.helper.hasClass('second-level') && !ui.helper.hasClass('mkd-sort-child')) {
                        ui.placeholder.removeClass('placeholder-sub');
                        ui.placeholder.addClass('placeholder');
		...
		if($('.mkd-backup-options-page-holder').length) {
			...
			mkdImportBtn.on('click', function(e) {
				...
				if (confirm('Are you sure, you want to import Options now?')) {
					...
					var data = {
						action: 'entre_mikado_import_theme_options',
						content: importValue,
						nonce: importNonce
					};
					$.ajax({
	...
	function mkdInitImportContent(){
		var mkdImportHolder   = $('.mkd-import-page-holder');
		
		if(mkdImportHolder.length) {
			var mkdImportBtn      = $('#mkd-import-demo-data');
			var confirmMessage = mkdImportHolder.data('confirm-message');
			mkdImportBtn.on('click', function(e) {
				e.preventDefault();

				if (confirm(confirmMessage)) {
					$('.mkd-import-load').css('display','block');
					...
					if(import_opt == 'content'){
						for( var i=1; i < 10; i++ ){
							var str;
							if (i < 10) str = 'entre_content_0'+i+'.xml';
							else str = 'entre_content_'+i+'.xml';
							jQuery.ajax({
								type: 'POST',
								url: ajaxurl,
								data: {
									action: 'mkd_core_data_import',
									xml: str,
									example: import_expl,
						...
						$.ajax({
							...
							url: ajaxurl,
							data: {
								action: 'mkd_core_widgets_import',
								example: import_expl
							},
							success: function(data, textStatus, XMLHttpRequest){
						...
						jQuery.ajax({
							...
							url: ajaxurl,
							data: {
								action: 'mkd_core_options_import',
								example: import_expl
							},
							success: function(data, textStatus, XMLHttpRequest){
								$('.progress-value').html((100) + '%');
    ...
    function mkdSelect2() {
        if ($('select.mkd-select2').length) {
            $('select.mkd-select2').select2({
                allowClear: true
            });
        }
    }

    function mkdInitGeocomplete() {
        var geo_inputs = $(".mkd-address-field");
        if(geo_inputs.length && !$('body').hasClass('mkd-empty-google-api')) {
