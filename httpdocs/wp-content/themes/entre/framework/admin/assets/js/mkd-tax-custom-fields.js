jQuery(document).ready(function ($) {
	...
	function mkd_tax_media_remove(button_class) {
		$('body').on('click', button_class, function () {
			var $this = $(this),
				parent = $this.closest('.form-field'),
				image = parent.find('.mkd-tax-custom-media-url');
			...
			if (image.val() !== '' && confirm('Are you sure you want to delete this file?')) {
				var result = $.ajax({
					...
					type: 'GET',
					data: {
						action: 'entre_mikado_tax_del_image',
	...
	function mkdInitTaxColorpicker() {
		var taxColor = $('.mkd-taxonomy-color-field');
		...
		if (settings.data) {
			...
			if ($.inArray('action=add-tag', queryStringArr) !== -1) {
				...
				if ($response !== "") {
					...
					$('.mkd-taxonomy-color-field').val('');
					$('.mkd-taxonomy-color-field').parents('.wp-picker-container').find('.wp-color-result').removeAttr('style');
				}
			}
		}
	});
	
	mkdInitTermIconSelectChange();
	
	function mkdInitTermIconSelectChange() {
