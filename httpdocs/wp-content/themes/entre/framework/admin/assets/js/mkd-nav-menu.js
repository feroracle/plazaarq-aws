(function($) {
	...
	$(document).ready(function() {
        mkdUpdateIconOptions();
		mkdInitAdditionalItemOptions();
	});

	/**
	 * Function that serializes additional menu item options in a single field.
	 */
	function mkdInitAdditionalItemOptions() {
		var navForm = $('#update-nav-menu');

		navForm.on('change', '[data-item-option]', function() {
			mkdGenerateSerializedString();
		});
	}

	function mkdGenerateSerializedString() {
		var dataArrayString = '',
		...
		var menuOptions = $('input[name="mkd_menu_options"]');
     ...
     */
    function mkdUpdateIconOptions() {
        var navForm = $('#update-nav-menu');

        navForm.on('change', '[data-icon-pack]', function() {
	        var thisItem = $(this),
		        chosenIconPack = thisItem.find('option:selected').val(),
		        iconDropdown = thisItem.parents('p').first().next('.mkd-icon-select-holder').find('select'),
