...
class EntreMikadoFramework {
	...
	private function __construct() {
		$this->mkdOptions         = EntreMikadoOptions::get_instance();
		$this->mkdMetaBoxes       = EntreMikadoMetaBoxes::get_instance();
		$this->mkdTaxonomyOptions = EntreMikadoTaxonomyOptions::get_instance();
		$this->mkdUserOptions     = EntreMikadoUserOptions::get_instance();
	}
	...
	public function setSkin( EntreMikadoSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class EntreMikadoSkinManager
 *
...
class EntreMikadoSkinManager {
	...
	/**
	 * @see EntreMikadoSkinManager::setSkin()
	 */
...
/**
 * Class EntreMikadoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class EntreMikadoSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			...
			include( entre_mikado_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo entre_mikado_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.mkdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'entre' ) ?>';
		</script>
	<?php }
...
class EntreMikadoOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $entre_mikado_options ) ) {
			return $entre_mikado_options[ $key ];
...
/*
   Class: EntreMikadoAdminPage
   A class that initializes Mikado Admin Page
*/
class EntreMikadoAdminPage implements iEntreMikadoLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title_label = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new EntreMikadoFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild( iEntreMikadoRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: EntreMikadoMetaBoxes
...
/*
   Class: EntreMikadoMetaBox
   A class that initializes Mikado Meta Box
*/
class EntreMikadoMetaBox implements iEntreMikadoLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $entre_mikado_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: EntreMikadoTaxonomyOptions
   A class that initializes EntreMikado Taxonomy Options
*/
class EntreMikadoTaxonomyOptions {
	private static $instance;
...
/*
   Class: EntreMikadoTaxonomyOption
   A class that initializes EntreMikado Taxonomy Option
*/
class EntreMikadoTaxonomyOption implements iEntreMikadoLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new EntreMikadoTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: EntreMikadoUserOptions
   A class that initializes EntreMikado User Options
*/
class EntreMikadoUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: EntreMikadoUserOption
   A class that initializes EntreMikado User Option
*/
class EntreMikadoUserOption implements iEntreMikadoLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new EntreMikadoUserFieldFactory();
		$this->scope   = $scope;
	}
...
if ( ! function_exists( 'entre_mikado_init_framework_variable' ) ) {
	function entre_mikado_init_framework_variable() {
		global $entre_mikado_Framework;
		
		$entre_mikado_Framework = EntreMikadoFramework::get_instance();
		$mkdSkinManager                 = new EntreMikadoSkinManager();
		$entre_mikado_Framework->setSkin( $mkdSkinManager->getSkin() );
	}
	
	add_action( 'entre_mikado_before_options_map', 'entre_mikado_init_framework_variable' );
}
?>