...
/*
  Class: EntreMikadoIconCollections
...
class EntreMikadoIconCollections
{
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new EntreMikadoIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new EntreMikadoIconsFontElegant("Font Elegant", "fe_icon"));
        $this->addIconCollection('ion_icons', new EntreMikadoIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linea_icons', new EntreMikadoLineaIcons('Linea Icons', 'linea_icon'));
        $this->addIconCollection('linear_icons', new EntreMikadoLinearIcons('Linear Icons', 'linear_icon'));
        $this->addIconCollection('simple_line_icons', new EntreMikadoSimpleLineIcons('Simple Line Icons', 'simple_line_icon'));
        $this->addIconCollection('dripicons', new EntreMikadoDripicons('Dripicons', 'dripicon'));
    }
    ...
    public function getIconsMetaBoxOrOption($attributes) {
        ...
        $options = array(
            'font_awesome'      => esc_html__('Font Awesome', 'entre'),
            'font_elegant'      => esc_html__('Font Elegant', 'entre'),
            'ion_icons'         => esc_html__('Ion Icons', 'entre'),
            'simple_line_icons' => esc_html__('Simple Line Icons', 'entre')
        );
        if ($scope == 'regular') {
            $options = array(
                'font_awesome'      => esc_html__('Font Awesome', 'entre'),
                'font_elegant'      => esc_html__('Font Elegant', 'entre'),
                'ion_icons'         => esc_html__('Ion Icons', 'entre'),
                'linea_icons'       => esc_html__('Linea Icons', 'entre'),
                'linear_icons'      => esc_html__('Linear Icons', 'entre'),
                'simple_line_icons' => esc_html__('Simple Line Icons', 'entre'),
                'dripicons'         => esc_html__('Dripicons', 'entre')
            );
        }
        ...
        foreach ($icon_collections as $collection_key => $collection_object) {
            ...
            $mkd_icon_pack_container = entre_mikado_add_admin_container(
                array(
                    'parent'          => $parent,
    ...
    public function getVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false) {
        ...
        $iconPackParams = array(
            'type'        => 'dropdown',
            'heading'     => esc_html__('Icon Pack', 'entre'),
    ...
    public function getIconWidgetParamsArray() {
        ...
        $iconPackParams[] = array(
            'type'    => 'dropdown',
            'name'    => 'icon_pack',
            'title'   => esc_html__('Icon Pack', 'entre'),
            'options' => array(
                'font_awesome'      => esc_html__('Font Awesome', 'entre'),
                'font_elegant'      => esc_html__('Font Elegant', 'entre'),
                'ion_icons'         => esc_html__('Ion Icons', 'entre'),
	...
	public function getSocialIconWidgetMultipleParamsArray($count) {
		...
		for ( $n = 1; $n <= $count; $n++ ) {
			...
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'entre' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_'.$n,
				'title' => sprintf( esc_html__( 'Link %s', 'entre' ), $n )
			);

			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_'.$n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'entre' ), $n ),
    ...
    public function getIconCollectionIcons(iEntreMikadoIconCollection $collection) {
    ...
    public function getIconCollectionsEmpty($no_empty_key = "") {
        $array = array();
        $array[$no_empty_key] = esc_html__( 'No Icon', 'entre');
        foreach ($this->iconCollections as $key => $collection) {
            $array[$key] = $collection->title;
        }
    ...
    public function getIconCollectionsVCEmpty() {
        $vc_array = array();
	    $vc_array[ esc_html__( 'No Icon', 'entre' ) ] = '';
        foreach ($this->iconCollections as $key => $collection) {
            $vc_array[$collection->title] = $key;
        }
    ...
    public function enqueueStyles() {
        if (is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach ($this->iconCollections as $collection_key => $collection_obj) {
                wp_enqueue_style('entre-mikado-' . $collection_key, $collection_obj->styleUrl);
            }
        }
    }
...
if (!function_exists('entre_mikado_activate_theme_icons')) {
    function entre_mikado_activate_theme_icons() {
        global $entre_mikado_IconCollections;
        $entre_mikado_IconCollections = EntreMikadoIconCollections::get_instance();
    }

    add_action('after_setup_theme', 'entre_mikado_activate_theme_icons');
}