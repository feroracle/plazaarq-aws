 ...
 */
if ( ! class_exists( 'EntreMikadoSidebar' ) ) {
	class EntreMikadoSidebar {
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'entre' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'entre-mikado-sidebar', MIKADO_ROOT . '/framework/admin/assets/js/mkd-sidebar.js' );
			wp_enqueue_style( 'entre-mikado-sidebar', MIKADO_ROOT . '/framework/admin/assets/css/mkd-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n  <form class='mkd-add-widget wp-block' method='POST' data-type='core/widget-area'>";
			echo "\n  <h3>" . esc_html($this->title) . "</h3>";
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_html__('Enter Name of the new Widget Area', 'entre') . "' name='mkd-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_html__('Add Widget Area', 'entre') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'entre_mikado_custom_widget_args', $args );
