...
if ( class_exists( 'EntreCoreClassWidget' ) ) {
	...
	class EntreMikadoIconWidget extends EntreCoreClassWidget {
		public function __construct() {
			parent::__construct(
				'mkd_icon_widget',
				esc_html__( 'Mikado Icon Widget', 'entre' ),
				array( 'description' => esc_html__( 'Add icons to widget areas', 'entre' ) )
			);
		...
		protected function setParams() {
			$this->params = array_merge(
				entre_mikado_icon_collections()->getIconWidgetParamsArray(),
				array(
					array(
						'type'  => 'textfield',
						'name'  => 'icon_text',
						'title' => esc_html__( 'Icon Text', 'entre' )
					),
					array(
						'type'  => 'textfield',
						'name'  => 'link',
						'title' => esc_html__( 'Link', 'entre' )
					),
					array(
						'type'    => 'dropdown',
						'name'    => 'target',
						'title'   => esc_html__( 'Target', 'entre' ),
						'options' => entre_mikado_get_link_target_array()
					),
					array(
						'type'  => 'textfield',
						'name'  => 'icon_size',
						'title' => esc_html__( 'Icon Size (px)', 'entre' )
					),
					array(
						'type'  => 'colorpicker',
						'name'  => 'icon_color',
		...
		public function widget( $args, $instance ) {
			$holder_classes = array( 'mkd-icon-widget-holder' );
			if ( ! empty( $instance['icon_hover_color'] ) ) {
				$holder_classes[] = 'mkd-icon-has-hover';
			}
			
			$data   = array();
			$data[] = ! empty( $instance['icon_hover_color'] ) ? entre_mikado_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
			$data   = implode( ' ', $data );
			...
			if ( ! empty( $instance['icon_size'] ) ) {
				$holder_styles[] = 'font-size: ' . entre_mikado_filter_px( $instance['icon_size'] ) . 'px';
			}
			...
			if ( ! empty( $instance['icon_pack'] ) ) {
				...
				if ( ! empty( $icon_class ) ) {
					...
					$icon_holder_html = '<span class="mkd-icon-element ' . esc_attr( $icon_class ) . '"></span>';
				}
			}
			
			$icon_text_html  = '';
			$icon_text_class = ! empty( $icon_holder_html ) ? '' : 'mkd-no-icon';
			if ( ! empty( $instance['icon_text'] ) ) {
				$icon_text_html = '<span class="mkd-icon-text ' . esc_attr( $icon_text_class ) . '">' . esc_html( $instance['icon_text'] ) . '</span>';
			}
			?>
			
			<a <?php entre_mikado_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?>
					href="<?php echo esc_url( $link ); ?>"
					target="<?php echo esc_attr( $target ); ?>" <?php echo entre_mikado_get_inline_style( $holder_styles ); ?>>
