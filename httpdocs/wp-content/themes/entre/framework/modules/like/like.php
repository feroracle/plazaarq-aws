...
class EntreMikadoLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_entre_mikado_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_entre_mikado_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'entre' );
		
		if ( isset( $_COOKIE[ 'mkd-like_' . $post->ID ] ) ) {
			$class = 'mkd-like liked';
			$title = esc_html__( 'You already like this!', 'entre' );
		}
...
if ( ! function_exists( 'entre_mikado_create_like' ) ) {
	function entre_mikado_create_like() {
		EntreMikadoLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'entre_mikado_create_like' );
}