...
namespace EntreMikado\Modules\Header\Lib;
/**
 * Class EntreMikadoHeaderConnector
 *
...
class EntreMikadoHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'entre_mikado_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'entre_mikado_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'entre_mikado_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'entre_mikado_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to entre_mikado_title_content_padding filter
	 *
	 * @return int
	 */
