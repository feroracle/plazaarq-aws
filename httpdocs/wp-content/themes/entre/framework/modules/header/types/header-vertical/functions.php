...
if ( ! function_exists( 'entre_mikado_register_header_vertical_type' ) ) {
	/**
	 * This function is used to register header type class for header factory file
	 */
	function entre_mikado_register_header_vertical_type( $header_types ) {
		$header_type = array(
			'header-vertical' => 'EntreMikado\Modules\Header\Types\HeaderVertical'
		);
...
if ( ! function_exists( 'entre_mikado_init_register_header_vertical_type' ) ) {
	/**
	 * This function is used to wait header-function.php file to init header object and then to init hook registration function above
	 */
	function entre_mikado_init_register_header_vertical_type() {
		add_filter( 'entre_mikado_register_header_type_class', 'entre_mikado_register_header_vertical_type' );
	}
	
	add_action( 'entre_mikado_before_header_function_init', 'entre_mikado_init_register_header_vertical_type' );
}

if ( ! function_exists( 'entre_mikado_include_header_vertical_menu' ) ) {
	/**
	 * Registers additional menu navigation for theme
	 */
	function entre_mikado_include_header_vertical_menu( $menus ) {
		$menus['vertical-navigation'] = esc_html__( 'Vertical Navigation', 'entre' );
		
		return $menus;
	}
	
	if ( entre_mikado_check_is_header_type_enabled( 'header-vertical' ) ) {
		add_filter( 'entre_mikado_register_headers_menu', 'entre_mikado_include_header_vertical_menu' );
	}
}

if ( ! function_exists( 'entre_mikado_register_header_vertical_widget_areas' ) ) {
	/**
	 * Registers additional widget areas for this header type
	 */
	function entre_mikado_register_header_vertical_widget_areas() {
		register_sidebar(
			array(
				'id'            => 'mkd-vertical-area',
				'name'          => esc_html__( 'Header Vertical Widget Area', 'entre' ),
	...
	if ( entre_mikado_check_is_header_type_enabled( 'header-vertical' ) ) {
		add_action( 'widgets_init', 'entre_mikado_register_header_vertical_widget_areas' );
	}
}

if ( ! function_exists( 'entre_mikado_get_header_vertical_widget_areas' ) ) {
	/**
	 * Loads header widgets area HTML
	 */
	function entre_mikado_get_header_vertical_widget_areas() {
		...
		if ( is_active_sidebar( 'mkd-vertical-area' ) && empty( $custom_vertical_header_widget_area ) ) {
...
if ( ! function_exists( 'entre_mikado_get_header_vertical_main_menu' ) ) {
	/**
	 * Loads vertical menu HTML
	 */
	function entre_mikado_get_header_vertical_main_menu() {
		entre_mikado_get_module_template_part( 'templates/vertical-navigation', 'header/types/header-vertical' );
	}
}

if ( ! function_exists( 'entre_mikado_vertical_header_holder_class' ) ) {
	/**
	 * Return holder class
	 */
	function entre_mikado_vertical_header_holder_class() {
...
if ( ! function_exists( 'entre_mikado_header_vertical_per_page_custom_styles' ) ) {
	/**
	 * Return header per page styles
	 */
	function entre_mikado_header_vertical_per_page_custom_styles( $style, $class_prefix, $page_id ) {
		$header_area_style    = array();
		$header_area_selector = array( $class_prefix . '.mkd-header-vertical .mkd-vertical-area-background' );
	...
	add_filter( 'entre_mikado_add_header_page_custom_style', 'entre_mikado_header_vertical_per_page_custom_styles', 10, 3 );
}