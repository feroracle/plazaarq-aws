...
if (!class_exists('EntreMikadoStickyNavigationWalker')) {
	class EntreMikadoStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($item->hide == ""){
                ...
                if($item->featured_icon !== ""){
                    $featured_icon .= '<i class="mkd-menu-featured-icon fa '.$item->featured_icon .'"></i>';
                }
				...
				if($args->has_children && $depth == 0) {
					$item_output .= '<i class="mkd-menu-arrow fa fa-angle-down"></i>';
				}
