<?php
$title_tag = isset($link_tag) ? $link_tag : 'h5';
$post_link_meta = get_post_meta(get_the_ID(), "mkd_post_link_link_meta", true );

if(entre_mikado_get_blog_module() == 'list') {
    $post_link = get_the_permalink();
} else {
    if(!empty($post_link_meta)) {
        $post_link = esc_html($post_link_meta);
    }
}
?>

<div class="mkd-post-link-holder">
    <div class="mkd-post-link-holder-inner">
        <<?php echo entre_mikado_escape_title_tag($title_tag);?> itemprop="name" class="mkd-link-title mkd-post-title">
        <?php if(isset($post_link) && $post_link != '') { ?>
        <a itemprop="url" href="<?php echo esc_url($post_link); ?>" title="<?php the_title_attribute(); ?>" target="_blank">
            <?php } ?>
            <?php echo get_the_title(); ?>
            <?php if(isset($post_link) && $post_link != '') { ?>
        </a>
        <?php } ?>
        </<?php echo entre_mikado_escape_title_tag($title_tag);?>>
    </div>
</div>