<?php
$blog_single_navigation = entre_mikado_options()->getOptionValue('blog_single_navigation') === 'no' ? false : true;
$blog_navigation_through_same_category = entre_mikado_options()->getOptionValue('blog_navigation_through_same_category') === 'no' ? false : true;
?>
			...
			<?php
				...
				$post_navigation = array(
					'prev' => array(
						...
						'label' => '<span class="mkd-blog-single-nav-label">'.esc_html__('previous', 'entre').'</span>'
					),
					'next' => array(
						'mark' => '<span class="mkd-blog-single-nav-mark ion-ios-arrow-thin-right"></span>',
						'label' => '<span class="mkd-blog-single-nav-label">'.esc_html__('next', 'entre').'</span>'
					)
				);
				...
				foreach (array('prev', 'next') as $nav_type) {
					if (isset($post_navigation[$nav_type]['post'])) { ?>
						<a itemprop="url" class="mkd-blog-single-<?php echo esc_attr($nav_type); ?>" href="<?php echo get_permalink($post_navigation[$nav_type]['post']->ID); ?>">
