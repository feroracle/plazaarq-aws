...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Mikado Product Info', 'entre' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'entre' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'entre' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'entre' ),
							'value'       => array(
								esc_html__( 'Title', 'entre' )          => 'title',
								esc_html__( 'Featured Image', 'entre' ) => 'featured_image',
								esc_html__( 'Category', 'entre' )       => 'category',
								esc_html__( 'Excerpt', 'entre' )        => 'excerpt',
								esc_html__( 'Price', 'entre' )          => 'price',
								esc_html__( 'Rating', 'entre' )         => 'rating',
								esc_html__( 'Add To Cart', 'entre' )    => 'add_to_cart',
								esc_html__( 'Tag', 'entre' )            => 'tag',
								esc_html__( 'Author', 'entre' )         => 'author',
								esc_html__( 'Date', 'entre' )           => 'date',
							),
							'admin_label' => true
						),
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'entre' ),
						...
						array(
							...
							'description' => esc_html__( 'Set title tag for product title element', 'entre' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'entre' ),
							'value'      => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'entre' ),
							'value'       => array_flip( entre_mikado_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'entre' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'entre' ),
							'value'      => array(
								esc_html__( 'Default', 'entre' ) => '',
								esc_html__( 'White', 'entre' )   => 'white',
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'entre' ),
							'value'      => array(
								esc_html__( 'Default', 'entre' ) => '',
								esc_html__( 'Small', 'entre' )   => 'small',
								esc_html__( 'Medium', 'entre' )  => 'medium',
								esc_html__( 'Large', 'entre' )   => 'large'
							),
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'entre' ) ),
							'group'      => esc_html__( 'Product Info Style', 'entre' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . entre_mikado_escape_title_tag( $title_tag ) . ' itemprop="name" class="mkd-pi-title entry-title" ' . entre_mikado_get_inline_style( $product_info_style ) . '>';
				$html .= '<a itemprop="url" href="' . esc_url( get_the_permalink( $product_id ) ) . '">' . esc_html( $title ) . '</a>';
			$html .= '</' . entre_mikado_escape_title_tag( $title_tag ) . '>';
		}
			...
			$html .= '<div class="mkd-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . entre_mikado_escape_title_tag( $params['category_tag'] ) . ' ' . entre_mikado_get_inline_style( $product_info_style ) . '>';
						$html .= '<a itemprop="url" class="mkd-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '">' . esc_html( $cat->name ) . '</a>';
						$html .= '</' . entre_mikado_escape_title_tag( $params['category_tag'] ) . '>';
					} else {
						$html .= '<a itemprop="url" class="mkd-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . entre_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="mkd-pi-excerpt"><p itemprop="description" class="mkd-pi-excerpt-item" ' . entre_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="mkd-pi-price" ' . entre_mikado_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="mkd-pi-rating" title="' . sprintf( esc_attr__( "Rated %s out of 5", "entre" ), $average ) . '" ' . entre_mikado_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		$html .= apply_filters( 'entre_mikado_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="mkd-pi-tag">';
				foreach ( $tags as $tag ) {
					$html .= '<a itemprop="url" class="mkd-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . entre_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
				}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="mkd-pi-author" ' . entre_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="mkd-pi-date" ' . entre_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'entre' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'entre' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'entre' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'entre' ) . ': ' . $product_id;
