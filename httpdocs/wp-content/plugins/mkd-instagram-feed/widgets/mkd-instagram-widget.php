...
class MikadoInstagramWidget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			array(
				'description' => esc_html__( 'Display your Instagram feed', 'mkd-instagram-feed' )
			)
		);
	...
	protected function setParams() {
		$this->params = array(
			...
			array(
				'name'    => 'type',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Type', 'mkd-instagram-feed' ),
				'options' => array(
					'gallery'  => esc_html__( 'Gallery', 'mkd-instagram-feed' ),
					'carousel' => esc_html__( 'Carousel', 'mkd-instagram-feed' )
				)
			),
			array(
				'name'    => 'number_of_cols',
			...
			array(
				...
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Space Between Items', 'mkd-instagram-feed' ),
				'options' => array(
					'normal' => esc_html__( 'Normal', 'mkd-instagram-feed' ),
					'small'  => esc_html__( 'Small', 'mkd-instagram-feed' ),
					'tiny'   => esc_html__( 'Tiny', 'mkd-instagram-feed' ),
					'no'     => esc_html__( 'No Space', 'mkd-instagram-feed' )
				)
			),
			array(
				'name'  => 'transient_time',
	...
	public function widget( $args, $instance ) {
		...
		if ( $type === 'carousel' ) {
			$widget_class = 'mkd-instagram-carousel mkd-owl-slider';
		...
		if ( is_array( $images_array ) && count( $images_array ) ) { ?>
			<ul class="mkd-instagram-feed clearfix mkd-col-<?php echo esc_attr( $number_of_cols ); ?> <?php echo esc_attr( $widget_class ); ?>" <?php echo entre_mikado_get_inline_attrs( $slider_data ); ?>>
				<?php
				foreach ( $images_array as $image ) { ?>
					<li>
						<a href="<?php echo esc_url( $instagram_api->getHelper()->getImageLink( $image ) ); ?>" target="_blank">
							<?php echo entre_mikado_kses_img( $instagram_api->getHelper()->getImageHTML( $image ) ); ?>
						</a>
					</li>
				<?php } ?>
			</ul>
