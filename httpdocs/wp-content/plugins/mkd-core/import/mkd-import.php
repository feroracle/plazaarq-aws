...
class MikadoCoreImport {
	...
	function __construct() {
		$this->revSliderFolder = 'mkd-rev-sliders';
	...
	public function import_options( $file ) {
		$options       = $this->file_options( $file );
		$result        = update_option( 'mkd_options_entre', $options );
	...
	function mkd_admin_import() {
		if ( mkd_core_theme_installed() ) {
			...
			$this->pagehook = add_submenu_page(
				...
				array( $entre_mikado_Framework->getSkin(), 'renderImport' )
			);
			
			add_action( 'admin_print_scripts-' . $this->pagehook, 'entre_mikado_enqueue_admin_scripts' );
			add_action( 'admin_print_styles-' . $this->pagehook, 'entre_mikado_enqueue_admin_styles' );
		}
	}
	...
	function mkd_update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( 'mkd_options_entre' );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( 'mkd_options_entre', $new_global_values );
		}
	}
	...
	function mkd_recalc_serialized_lengths( $sObject ) {
		$ret = preg_replace_callback( '!s:(\d+):"(.*?)";!', array($this, 'mkd_recalc_serialized_lengths_callback'), $sObject );
