...
if ( ! function_exists( 'mkd_core_add_admin_shortcodes_styles' ) ) {
	/**
	 * Function that includes shortcodes core styles for admin
	 */
	function mkd_core_add_admin_shortcodes_styles() {
		
		//include shortcode styles for Visual Composer
		wp_enqueue_style( 'entre-mikado-core-vc-shortcodes', MIKADO_CORE_ASSETS_URL_PATH . '/css/admin/mkd-vc-shortcodes.css' );
	}
	
	add_action( 'entre_mikado_admin_scripts_init', 'mkd_core_add_admin_shortcodes_styles' );
}

if ( ! function_exists( 'mkd_core_add_admin_shortcodes_custom_styles' ) ) {
	/**
	 * Function that print custom vc shortcodes style
	 */
	function mkd_core_add_admin_shortcodes_custom_styles() {
		$style                  = apply_filters( 'mkd_core_filter_add_vc_shortcodes_custom_style', $style = '' );
		$shortcodes_icon_styles = array();
		$shortcode_icon_size    = 32;
		$shortcode_position     = 0;
		
		$shortcodes_icon_class_array = apply_filters( 'mkd_core_filter_add_vc_shortcodes_custom_icon_class', $shortcodes_icon_class_array = array() );
		...
		if ( ! empty( $style ) ) {
			wp_add_inline_style( 'entre-mikado-core-vc-shortcodes', $style );
		}
	}
	
	add_action( 'entre_mikado_admin_scripts_init', 'mkd_core_add_admin_shortcodes_custom_styles' );
}