...
class ItemShowcase implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'      => esc_html__( 'Mikado Item Showcase', 'mkd-core' ),
					'base'      => $this->base,
					...
					'params'    => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Image Top Offset', 'mkd-core' ),
							'value'       => '-100px',
							'save_always' => true
						)
					)
	...
	public function render( $atts, $content = null ) {
		...
		if ( ! empty( $image_top_offset ) ) {
			$item_image_style = 'margin-top: ' . entre_mikado_filter_px( $image_top_offset ) . 'px';
		}
		
		$html = '<div class="mkd-item-showcase-holder clearfix">';
			$html .= do_shortcode( $content );
			if ( ! empty( $item_image ) ) {
				$html .= '<div class="mkd-is-image" ' . entre_mikado_get_inline_style( $item_image_style ) . '>';
