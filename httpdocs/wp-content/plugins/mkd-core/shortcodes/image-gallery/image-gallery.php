<?php
namespace MikadoCore\CPT\Shortcodes\ImageGallery;
...
class ImageGallery implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Mikado Image Gallery', 'mkd-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							'type'        => 'dropdown',
							'param_name'  => 'type',
							'heading'     => esc_html__( 'Gallery Type', 'mkd-core' ),
							'value'       => array(
								esc_html__( 'Image Grid', 'mkd-core' ) => 'grid',
						...
						array(
							...
							'heading'     => esc_html__( 'Custom Links', 'mkd-core' ),
							'description' => esc_html__( 'Delimit links by comma', 'mkd-core' ),
						...
						array(
							...
							'heading'     => esc_html__( 'Enable Slider Padding', 'mkd-core' ),
							'description' => esc_html__( 'Padding left and right on stage (can see neighbours).', 'mkd-core' ),
							'value'       => array_flip( entre_mikado_get_yes_no_select_array( false ) ),
							'save_always' => true,
						...
						array(
							'type'        => 'dropdown',
							'param_name'  => 'fullscreen_carousel',
							'heading'     => esc_html__( 'Enable FullScreen Carousel', 'mkd-core' ),
							'value'       => array_flip( entre_mikado_get_yes_no_select_array( false ) ),
							'save_always' => true,
							'dependency'  => array( 'element' => 'type', 'value' => array( 'carousel' ) ),
							'group'       => esc_html__( 'Slider Settings', 'mkd-core' )
						...
						array(
							...
							'heading'     => esc_html__( 'Enable Slider Navigation Arrows', 'mkd-core' ),
							'value'       => array_flip( entre_mikado_get_yes_no_select_array( false, true ) ),
							'save_always' => true,
							'dependency'  => array( 'element' => 'type', 'value' => array( 'slider', 'carousel' ) ),
							'group'       => esc_html__( 'Slider Settings', 'mkd-core' )
						...
						array(
							...
							'heading'     => esc_html__( 'Enable Slider Pagination', 'mkd-core' ),
							'value'       => array_flip( entre_mikado_get_yes_no_select_array( false, true ) ),
							'save_always' => true,
							'dependency'  => array( 'element' => 'type', 'value' => array( 'slider', 'carousel' ) ),
							'group'       => esc_html__( 'Slider Settings', 'mkd-core' )
	...
	public function render( $atts, $content = null ) {
		...
		$html = mkd_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'image-gallery', '', $params );
	...
	private function getHolderClasses( $params, $args ) {
		...
		$holderClasses[] = ! empty( $params['type'] ) ? 'mkd-ig-' . $params['type'] . '-type' : 'mkd-ig-' . $args['type'] . '-type';
		$holderClasses[] = ! empty( $params['space_between_items'] ) ? 'mkd-' . $params['space_between_items'] . '-space' : 'mkd-' . $args['space_between_items'] . '-space';
		$holderClasses[] = $params['enable_image_shadow'] === 'yes' ? 'mkd-has-shadow' : '';
	...
	private function getInnerClasses( $params, $args ) {
		...
		$holderClasses[] = $params['type'] === 'masonry' ? 'mkd-ig-masonry' : 'mkd-ig-grid';
		$holderClasses[] = ! empty( $params['number_of_columns'] ) ? 'mkd-ig-' . $params['number_of_columns'] . '-columns' : 'mkd-ig-' . $args['number_of_columns'] . '-columns';
	...
	private function getSliderData( $params ) {
		...
		$slider_data['data-fullscreen-carousel']    = ! empty( $params['fullscreen_carousel'] ) ? $params['fullscreen_carousel'] : '';
