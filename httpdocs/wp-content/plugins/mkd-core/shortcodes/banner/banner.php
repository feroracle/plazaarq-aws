...
class Banner implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Image Overlay Color', 'mkd-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'hover_behavior',
							'heading'     => esc_html__( 'Hover Behavior', 'mkd-core' ),
							'value'       => array(
								esc_html__( 'Visible on Hover', 'mkd-core' )   => 'mkd-visible-on-hover',
								esc_html__( 'Visible on Default', 'mkd-core' ) => 'mkd-visible-on-default',
								esc_html__( 'Disabled', 'mkd-core' )           => 'mkd-disabled'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Position', 'mkd-core' ),
							'value'       => array(
								esc_html__( 'Default', 'mkd-core' )  => 'default',
								esc_html__( 'Centered', 'mkd-core' ) => 'centered'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Content Padding', 'mkd-core' ),
							'description' => esc_html__( 'Please insert padding in format top right bottom left', 'mkd-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'subtitle',
						...
						array(
							...
							'heading'    => esc_html__( 'Link Text Color', 'mkd-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'link_top_margin',
							'heading'    => esc_html__( 'Link Text Top Margin (px)', 'mkd-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		$args   = array(
			...
			'image'                => '',
			'overlay_color'        => '',
			'hover_behavior'       => 'mkd-visible-on-hover',
	...
	private function getHolderClasses( $params, $args ) {
		...
		$holderClasses[] = ! empty( $params['info_position'] ) ? 'mkd-banner-info-' . $params['info_position'] : 'mkd-banner-info-' . $args['info_position'];
			...
			if ( ! empty( $title_light_words ) ) {
				foreach ( $light_words as $value ) {
					if ( ! empty( $split_title[ $value - 1 ] ) ) {
						$split_title[ $value - 1 ] = '<span class="mkd-banner-title-light">' . $split_title[ $value - 1 ] . '</span>';
					}
				}
			}
	...
	private function getTitleStyles( $params ) {
		...
		if ( ! empty( $params['title_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . entre_mikado_filter_px( $params['title_top_margin'] ) . 'px';
		}
	...
	private function getLinkStyles( $params ) {
		...
		if ( ! empty( $params['link_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . entre_mikado_filter_px( $params['link_top_margin'] ) . 'px';
		}
