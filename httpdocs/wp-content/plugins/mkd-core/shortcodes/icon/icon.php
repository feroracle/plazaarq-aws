...
class Icon implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    'name'                      => esc_html__( 'Mikado Icon', 'mkd-core' ),
				    'base'                      => $this->base,
				    ...
				    'params'                    => array_merge(
					    ...
					    \EntreMikadoIconCollections::get_instance()->getVCParamsArray(),
					    array(
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'size',
							    ...
							    'value'      => array(
								    ...
								    esc_html__( 'Huge', 'mkd-core' )   => 'mkd-icon-huge'
							    )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'custom_size',
							    'heading'    => esc_html__( 'Custom Size (px)', 'mkd-core' )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'type',
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Border Radius', 'mkd-core' ),
							    'description' => esc_html__( 'Please insert border radius(Rounded corners) in px. For example: 4 ', 'mkd-core' ),
							    'dependency'  => array( 'element' => 'type', 'value' => array( 'mkd-square' ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'shape_size',
							    'heading'    => esc_html__( 'Shape Size (px)', 'mkd-core' ),
							    'dependency' => array( 'element' => 'type', 'value' => array( 'mkd-circle', 'mkd-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Border Width', 'mkd-core' ),
							    'dependency' => array( 'element' => 'type', 'value'   => array( 'mkd-circle', 'mkd-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Hover Icon Color', 'mkd-core' )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Use Link as Anchor', 'mkd-core' ),
							    'value'       => array( 'Use this icon as Anchor?' => 'yes' ),
							    'description' => esc_html__( 'Check this box to use icon as anchor link (eg. #contact)', 'mkd-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = mkd_core_get_shortcode_module_template_part( 'templates/icon', 'icon', '', $params );
		
		return $html;
	}
	
	private function generateIconHolderClasses( $params ) {
		$holderClasses = array( 'mkd-icon-shortcode', $params['type'] );
		
		$holderClasses[] = ! empty( $params['custom_class'] ) ? esc_attr( $params['custom_class'] ) : '';
		$holderClasses[] = $params['icon_animation'] == 'yes' ? 'mkd-icon-animation' : '';
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'mkd-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'mkd-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . entre_mikado_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'mkd-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . entre_mikado_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . entre_mikado_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . entre_mikado_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . entre_mikado_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'mkd-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . entre_mikado_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
	...
	private function generateIconHolderData( $params ) {
		...
		if ( isset( $params['type'] ) && $params['type'] !== 'mkd-normal' ) {
