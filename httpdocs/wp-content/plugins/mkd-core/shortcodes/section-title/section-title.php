...
class SectionTitle implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						...
						array(
							...
							'group'       => esc_html__( 'Title Style', 'mkd-core' )
						),
						array(
							'type'       => 'colorpicker',
						...
						array(
							...
							'heading'     => esc_html__( 'Words with Bold Font Weight', 'mkd-core' ),
							'description' => esc_html__( 'Enter the positions of the words you would like to display in a "bold" font weight. Separate the positions with commas (e.g. if you would like the first, second, and third word to have a light font weight, you would enter "1,2,3")', 'mkd-core' ),
							'dependency'  => array( 'element' => 'title', 'not_empty' => true ),
							'group'       => esc_html__( 'Title Style', 'mkd-core' )
						),
						array(
							...
							'heading'     => esc_html__( 'Words with Light Font Weight', 'mkd-core' ),
							'description' => esc_html__( 'Enter the positions of the words you would like to display in a "light" font weight. Separate the positions with commas (e.g. if you would like the first, third, and fourth word to have a light font weight, you would enter "1,3,4")', 'mkd-core' ),
							'dependency'  => array( 'element' => 'title', 'not_empty' => true ),
							'group'       => esc_html__( 'Title Style', 'mkd-core' )
						),
						array(
							...
							'description' => esc_html__( 'Enter the position of the word after which you would like to create a line break (e.g. if you would like the line break after the 3rd word, you would enter "3")', 'mkd-core' ),
							'dependency'  => array( 'element' => 'title', 'not_empty' => true ),
							'group'       => esc_html__( 'Title Style', 'mkd-core' )
						),
						array(
						...
						array(
							...
							'group'       => esc_html__( 'Subtitle Style', 'mkd-core' )
						),
						array(
							'type'       => 'colorpicker',
						...
						array(
							...
							'heading'    => esc_html__( 'Separator Color', 'mkd-core' ),
							'dependency' => array( 'element' => 'subtitle', 'not_empty' => true ),
							'group'      => esc_html__( 'Subtitle Style', 'mkd-core' )
						),
						array(
						...
						array(
							'type'        => 'dropdown',
							'param_name'  => 'text_tag',
							'heading'     => esc_html__( 'Text Tag', 'mkd-core' ),
							'value'       => array_flip( entre_mikado_get_title_tag( true, array( 'p' => 'p' ) ) ),
							'save_always' => true,
							'dependency'  => array( 'element' => 'text', 'not_empty' => true ),
							'group'       => esc_html__( 'Text Style', 'mkd-core' )
						),
						array(
							'type'       => 'colorpicker',
	...
	private function getHolderClasses( $params, $args ) {
		...
		$holderClasses[] = $params['disable_break_words'] === 'yes' ? 'mkd-st-disable-title-break' : '';
		...
		if ( ! empty( $title ) && ! empty( $title_break_words ) ) {
			...
			if ( ! empty( $title_bold_words ) ) {
				foreach ( $bold_words as $value ) {
					if ( ! empty( $split_title[ $value - 1 ] ) ) {
						$split_title[ $value - 1 ] = '<span class="mkd-st-title-bold">' . $split_title[ $value - 1 ] . '</span>';
					}
				}
			}
			...
			if ( ! empty( $title_light_words ) ) {
				foreach ( $light_words as $value ) {
					if ( ! empty( $split_title[ $value - 1 ] ) ) {
						$split_title[ $value - 1 ] = '<span class="mkd-st-title-light">' . $split_title[ $value - 1 ] . '</span>';
					}
				}
			}
	...
	private function getTextStyles( $params ) {
		...
		if ( ! empty( $params['text_font_size'] ) ) {
			$styles[] = 'font-size: ' . entre_mikado_filter_px( $params['text_font_size'] ) . 'px';
		}
		
		if ( ! empty( $params['text_line_height'] ) ) {
			$styles[] = 'line-height: ' . entre_mikado_filter_px( $params['text_line_height'] ) . 'px';
		}
		...
		if ( $params['text_margin'] !== '' ) {
			$styles[] = 'margin-top: ' . entre_mikado_filter_px( $params['text_margin'] ) . 'px';
		}
