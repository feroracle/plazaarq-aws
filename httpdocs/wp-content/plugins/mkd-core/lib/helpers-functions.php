...
if ( ! function_exists( 'mkd_core_get_cpt_shortcode_module_template_part' ) ) {
	 ...
	 * @return html
	 */
	function mkd_core_get_cpt_shortcode_module_template_part( $post_type, $shortcode, $template, $slug = '', $params = array(), $additional_params = array() ) {

		//HTML Content from template
		$html          = '';
		$template_path = MIKADO_CORE_CPT_PATH . '/' . $post_type . '/shortcodes/' . $shortcode . '/templates';
...
if ( ! function_exists( 'mkd_core_return_cpt_single_module_template_part' ) ) {
	/**
	 * Loads module template part.
	 *
	 * @param string $cpt_name name of the cpt folder
	 ...
	 * @return html
	 */
	function mkd_core_return_cpt_single_module_template_part( $template, $cpt_name, $slug = '', $params = array() ) {
...
if ( ! function_exists( 'entre_mikado_add_user_custom_fields' ) ) {
	/**
	 * Function creates custom social fields for users
	 *
	 * return $user_contact
	 */
	function entre_mikado_add_user_custom_fields( $user_contact ) {
		/**
		...
		$user_contact['userposition'] = esc_html__( 'Position', 'mkd-core' );

		return $user_contact;
	}

	add_filter( 'user_contactmethods', 'entre_mikado_add_user_custom_fields' );
}

if ( ! function_exists( 'mkd_core_set_open_graph_meta' ) ) {
	/*
	 * Function that echoes open graph meta tags if enabled
	 */
	function mkd_core_set_open_graph_meta() {

		if ( entre_mikado_option_get_value( 'enable_open_graph' ) === 'yes' ) {
			...
			if ( entre_mikado_is_woocommerce_installed() && ( function_exists( 'is_shop' ) && is_shop() ) ) {
			...
			if ( ! empty( $id ) ) {
				...
				if ( get_post_thumbnail_id( $id ) !== '' ) {
					...
					$image = entre_mikado_option_get_value( 'open_graph_image' );
				}
			} else {
				global $wp;
	...
	add_action( 'entre_mikado_header_meta', 'mkd_core_set_open_graph_meta' );
}

/* Function for adding custom meta boxes hooked to default action */
if ( class_exists( 'WP_Block_Type' ) && defined( 'MIKADO_ROOT' ) ) {
	add_action( 'admin_head', 'entre_mikado_meta_box_add' );
} else {
	add_action( 'add_meta_boxes', 'entre_mikado_meta_box_add' );
}

if (!function_exists('entre_mikado_create_meta_box_handler')) {
	function entre_mikado_create_meta_box_handler( $box, $key, $screen ) {
		add_meta_box(
			'mkdf-meta-box-' . $key,
			$box->title,
			'entre_mikado_render_meta_box',
			$screen,
			'advanced',
			'high',
			array( 'box' => $box )
