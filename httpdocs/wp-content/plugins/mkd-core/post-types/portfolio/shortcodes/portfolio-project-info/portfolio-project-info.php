...
class PortfolioProjectInfo implements Lib\ShortcodeInterface {
	...
	public function __construct() {
		$this->base = 'mkd_portfolio_project_info';
		
		add_action( 'vc_before_init', array( $this, 'vcMap' ) );
		
		//Portfolio project id filter
		add_filter( 'vc_autocomplete_mkd_portfolio_project_info_project_id_callback', array( &$this, 'portfolioIdAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array
		
		//Portfolio project id render
		add_filter( 'vc_autocomplete_mkd_portfolio_project_info_project_id_render', array( &$this, 'portfolioIdAutocompleteRender', ), 10, 1 ); // Render exact portfolio. Must return an array (label,value)
	}
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'     => esc_html__( 'Mikado Portfolio Project Info', 'mkd-core' ),
					'base'     => $this->getBase(),
					...
					'params'   => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Project', 'mkd-core' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'description' => esc_html__( 'If you left this field empty then project ID will be of the current page', 'mkd-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'project_info_type',
							'heading'     => esc_html__( 'Project Info Type', 'mkd-core' ),
							'value'       => array(
								esc_html__( 'Title', 'mkd-core' )          => 'title',
								esc_html__( 'Category', 'mkd-core' )       => 'category',
								esc_html__( 'Tag', 'mkd-core' )            => 'tag',
								esc_html__( 'Author', 'mkd-core' )         => 'author',
								esc_html__( 'Date', 'mkd-core' )           => 'date',
								esc_html__( 'Featured Image', 'mkd-core' ) => 'image'
							),
							'admin_label' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Project Title Tag', 'mkd-core' ),
							'value'       => array_flip( entre_mikado_get_title_tag( true, array( 'p' => 'p' ) ) ),
							'description' => esc_html__( 'Set title tag for project title element', 'mkd-core' ),
							'dependency'  => array( 'element' => 'project_info_type', 'value' => array( 'title' ) )
						),
						array(
							'type'        => 'textfield',
							'param_name'  => 'project_info_title',
							'heading'     => esc_html__( 'Project Info Label', 'mkd-core' ),
							'description' => esc_html__( 'Add project info label before project info element/s', 'mkd-core' )
						),
						array(
							'type'       => 'dropdown',
							'param_name' => 'project_info_title_tag',
							'heading'    => esc_html__( 'Project Info Label Tag', 'mkd-core' ),
							'value'      => array_flip( entre_mikado_get_title_tag( true, array( 'p' => 'p' ) ) ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = '<div class="mkd-portfolio-project-info">';
		
		if ( ! empty( $project_info_title ) ) {
			$html .= '<' . esc_attr( $project_info_title_tag ) . ' class="mkd-ppi-label">' . esc_html( $project_info_title ) . '</' . esc_attr( $project_info_title_tag ) . '>';
		}
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="mkd-ppi-title entry-title">';
	...
	public function getItemCategoryHtml( $params ) {
		...
		if ( ! empty( $categories ) ) {
			$html = '<div class="mkd-ppi-category">';
			foreach ( $categories as $cat ) {
				$html .= '<a itemprop="url" class="mkd-ppi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '">' . esc_html( $cat->name ) . '</a>';
			}
			$html .= '</div>';
		}
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="mkd-ppi-tag">';
			foreach ( $tags as $tag ) {
				$html .= '<a itemprop="url" class="mkd-ppi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '">' . esc_html( $tag->name ) . '</a>';
			}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="mkd-ppi-author">' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="mkd-ppi-date">' . esc_html( $date ) . '</div>';
		}
	...
	public function getItemImageHtml( $params ) {
		...
		if ( ! empty( $image ) ) {
			$html = '<a itemprop="url" class="mkd-ppi-image" href="' . esc_url( get_the_permalink( $project_id ) ) . '">' . $image . '</a>';
		}
